...
auto AbstractMenu::get_buttons() -> std::vector<Button>& {
    return buttons;
}


template <typename T>
auto AbstractMenu::configure_component(T& component, sf::Vector2f position, float scale) -> void {
...
auto AbstractMenu::create_button(const std::string &text, const sf::Font& font, sf::Vector2f position) -> Button {
...
auto AbstractMenu::create_button(const std::string &text, sf::Vector2f position) -> Button {
    auto button = Button(button_active_texture, button_inactive_texture, font, text, 24);
    configure_component(button, position, menuScale);
    return button;
}
...
void AbstractMenu::draw(sf::RenderTarget &target, sf::RenderStates states) const {
