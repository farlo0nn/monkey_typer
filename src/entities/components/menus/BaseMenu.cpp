...
BaseMenu::BaseMenu()
    : background_texture("assets/ui/menu/mainMenuBanner.png"),
      ...
      fontSize(48)
{
    configureComponent(background, {WINDOW_SIZE.x/2, WINDOW_SIZE.y/2}, 4.5);
}

auto BaseMenu::getButtons() -> std::vector<Button>& {
    return buttons;
}


template <typename T>
auto BaseMenu::configureComponent(T& component, sf::Vector2f position, float scale) -> void {
...
auto BaseMenu::createButton(const std::string &text, const sf::Font& font, sf::Vector2f position) -> Button {
    auto button = Button(button_active_texture, button_inactive_texture, font, text, 24);
    configureComponent(button, position, menuScale);
    return button;
}

auto BaseMenu::createButton(const std::string &text, sf::Vector2f position) -> Button {
    auto button = Button(button_active_texture, button_inactive_texture, font, text, 24);
    configureComponent(button, position, menuScale);
    return button;
}
